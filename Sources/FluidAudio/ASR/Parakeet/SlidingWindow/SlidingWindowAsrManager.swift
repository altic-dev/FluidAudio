...
public actor SlidingWindowAsrManager {
    ...
    private var accumulatedTokenWindows: [TokenWindow] = []
    ...
    public func start(models: AsrModels, source: AudioSource = .microphone) async throws {
        ...
        accumulatedTokenWindows.removeAll()
    ...
    public func finish() async throws -> String {
        ...
        } else if let asrManager = asrManager, !accumulatedTokenWindows.isEmpty {
            let finalResult = await asrManager.processTranscriptionResult(
                tokenIds: accumulatedTokenWindows.map(\.token),
                timestamps: accumulatedTokenWindows.map(\.timestamp),
                confidences: accumulatedTokenWindows.map(\.confidence),
                tokenDurations: accumulatedTokenWindows.map(\.duration),
    ...
    private func processWindow(
        ...
        do {
            ...
            let previousTokens = accumulatedTokenWindows.map(\.token)
            let (tokens, timestamps, confidences, durations, _) = try await asrManager.transcribeStreamingChunk(
                windowSamples,
                source: audioSource,
            ...
            // Update state
            let tokenDurations =
                durations.count == tokens.count ? durations : Array(repeating: 0, count: tokens.count)
            let windowTokens: [TokenWindow] = zip(
                zip(zip(tokens, adjustedTimestamps), confidences), tokenDurations
            ).map {
                (token: $0.0.0.0, timestamp: $0.0.0.1, confidence: $0.0.1, duration: $0.1)
            }
            accumulatedTokenWindows = Self.mergeTokenWindows(accumulatedTokenWindows, windowTokens)
            accumulatedTokens = accumulatedTokenWindows.map(\.token)
    ...
    private static func mergeTokenWindows(
        _ left: [TokenWindow],
        _ right: [TokenWindow]
    ) -> [TokenWindow] {
        ...
        let frameDuration = Double(ASRConstants.samplesPerEncoderFrame) / 16_000.0
        ...
        func endTime(of token: TokenWindow) -> Double {
            startTime(of: token) + max(Double(token.duration) * frameDuration, frameDuration)
        }
        ...
        let overlapRight: [IndexedToken] = right.enumerated().compactMap { offset, token in
            ...
            return IndexedToken(index: offset, token: token, start: start, end: endTime(of: token))
        }
    ...
    private static func findBestContiguousPairs(
        ...
        for i in 0..<overlapLeft.count {
            for j in 0..<overlapRight.count where tokensMatch(overlapLeft[i], overlapRight[j], tolerance: tolerance) {
                var current: [(Int, Int)] = []
                var leftIndex = i
                var rightIndex = j

                while leftIndex < overlapLeft.count && rightIndex < overlapRight.count {
                    let nextLeft = overlapLeft[leftIndex]
                    let nextRight = overlapRight[rightIndex]

                    guard tokensMatch(nextLeft, nextRight, tolerance: tolerance) else {
                        break
                    }

                    current.append((leftIndex, rightIndex))
                    leftIndex += 1
                    rightIndex += 1
                }
    ...
    private static func tokensMatch(_ left: IndexedToken, _ right: IndexedToken, tolerance: Double) -> Bool {
        ...
        return abs(left.start - right.start) < tolerance
    }
    ...
    private static func mergeUsingMatches(
        ...
        for index in 0..<matches.count {
            let leftIndex = leftIndices[index]
            let rightIndex = rightIndices[index]
            result.append(left[leftIndex])

            guard index < matches.count - 1 else { continue }

            let nextLeftIndex = leftIndices[index + 1]
            let nextRightIndex = rightIndices[index + 1]
